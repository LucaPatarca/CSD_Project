...
class MergeMapNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(OccupancyGrid, '/pippo/map', self.map1_callback, 10)
        self.subscription = self.create_subscription(OccupancyGrid, '/pluto/map', self.map2_callback, 10)
        self.map1 = None
        self.map2 = None
